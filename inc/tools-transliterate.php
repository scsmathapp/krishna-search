 ...
 *   Transliterated text.
 */
private static function _transliteration_process($string, $unknown = '?', $source_langcode = 'en') {
...
private static function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
  ...
  if (!isset($map[$bank][$langcode])) {
    $file = JTransliteration::_get_path() . '/transliterate-data/' . sprintf('x%02x', $bank) . '.php';
    if (file_exists($file)) {
      include $file;
